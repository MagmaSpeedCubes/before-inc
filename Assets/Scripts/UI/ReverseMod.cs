...
public class ReverseMod : MonoBehaviour, IPointerDownHandler, IPointerUpHandler
{
    
    private static ReverseMod instance;
    ...
    [SerializeField] private TextMeshProUGUI ReverseModSubtextObject;
    ...
    private int state = 2; // 0 = locked, 1 = unselected, 2 = selected, 3 = reversed
    ...
    public void selectReverseMod()
    {
        if(!modSelected){
            ...
            ReverseModSubtextObject.text = reverseModSubtext;

            StartCoroutine(revealText(ReverseModTextObject, 0.5f));
            StartCoroutine(revealText(ReverseModSubtextObject, 0.5f));
